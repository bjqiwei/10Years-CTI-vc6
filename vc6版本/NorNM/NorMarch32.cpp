...
BOOL CNorMarch32::Nor32_InitSystem(CListCtrl *plstChannel, CListBox *plistSystem, CListCtrl *plistScript, CListBox *pCommList, CListCtrl *plistTCPIP, CListCtrl *plstConfer, CListCtrl *plstFax)
{
	...
	int k = tpi_GetCardPass( 0 );
	int l = tpi_GetCardUserNo(0);
...
inline int CNorMarch32::Nor32_GetCifChnlType(int nDevChnlType)
{
	switch(nDevChnlType)
	{
		case CH_DIGITAL_TRUNK ://= 0,	//数字中继通道
			return CF_CHTYPE_DTNO7;
		case CH_ANALOG_TRUNK://		//模拟中继通道(市话线)通道
			return CF_CHTYPE_TRUNK;
		case CH_VOIP://				//VoIP语音压缩解压通道
			return CF_CHTYPE_VIP;
		case CH_HIGH_IMPEDANCE://		//高阻监控通道(模块)
			return CF_CHTYPE_RECORD;
		case CH_USER://			//内线坐席通道
			return CF_CHTYPE_USER;
		case CH_VOICE://		//语音通道
		case CH_CONFERENCE://			//电话会议通道
		case CH_PCM://
		case CH_MFC://
		case CH_DTMF://
		case CH_MVIP://
		case CH_EMPTY://				//表示空通道(模拟卡、座席卡未插模块)
		case CH_SPEAKER://
		case CH_MAX://
		default:
...
void CNorMarch32::Nor32_OnTrunkCallIn(int nIndex, char *pszCallerID, char *pszCalledID)
{
	...
	if( CF_CHTYPE_TRUNK == pChnlObj->m_nChanelType || CF_CHTYPE_RECORD == pChnlObj->m_nChanelType )
	{
		pChnlObj->m_strCalledID= pChnlObj->m_strLocalDN ;
	}
	pChnlObj->m_nCallType = 1;
	...
	if( NODETYPE_WAITCALL == pNodeObj->m_nType )
	{
		...
		Nor32_MyOFFHook( nIndex, pObj->m_bAnswerCall );


		m_pPubCObj->PC_ShowChannelState( nIndex, "Busy" );
		pChnlObj->m_nState =  CHANNEL_STATE_BUSY;
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT );
		return;
	}
...
BOOL CNorMarch32::Nor32_EventCheck_EventMsg(int nIndex, CEventCheckObj *pObj)
{
	...
	case EVENTTYPE_OFFHOOK:
		...
		if( CF_CHTYPE_USER == pChnlInfo->m_nChanelType )
		{
			pChnlInfo->m_nState	   = CHANNEL_STATE_BUSY;
	...
	case EVENTTYPE_RECFAXTONE:
		...
		break;
	case EVENTTYPE_APPEXIT:
		pChnlInfo->m_nEventType = EVENTTYPE_NOTHING;
		if( pObj->m_bAppExit )
		{
			
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_APPEXIT);
			return TRUE;
		}
		break;
	case EVENTTYPE_FAXSENDTONE:
		pChnlInfo->m_nEventType = EVENTTYPE_NOTHING;
		if( pObj->m_bFaxSendTone )
		{
			
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_FAXSENDTONE);
			return TRUE;
		}
		break;
	...
	case 1:
		{
			...
			if( g_pD08a->CompareVarKey( 1,nPlayWay ) )
			//if( 1 == nPlayWay )//会议放音
			{
...
void CNorMarch32::Nor32_MyStopPlay(int nIndex )
{
	...
	if( g_pD08a->CompareVarKey( 1,pChnlInfo->m_nPlayType ) ) 
...
void CNorMarch32::Nor32_MyStopRecord(int nIndex  )
{
	...
	int channelType = Nor32_GetDevChnlType( pChnlInfo->m_nChanelType );
	int channelID = pChnlInfo->m_nChannel ;
	if( g_pD08a->CompareVarKey( 1,pChnlInfo->m_nRecordType ) ) 
...
BOOL CNorMarch32::Nor32_MyStartPlayFile(  int nIndex, CString strFile, int nConNo, BOOL bPlayLoop,int nPlayType/*放音类型：0普通 1会议*/ )
{
	...
	if( 1 == nPlayType )//会议放音
	{
		...
		if( g_pD08a->CompareVarKey( 0, tpi_PlayFile( 5, nConNo, pszFileName, 0, 0 ) ) ) 
	...
	else//通道放音
	{
		...
		{
			if( g_pD08a->CompareVarKey( 0, tpi_PlayFile( channelType, channelID, pszFileName, 0, 0 ) ) ) 
...
void CNorMarch32::Nor32_CallHold_PickUp(int nIndex, CCallHoldObj *pObj)
{
	...
	int nChnlLink = -1;
	if( g_pD08a->CompareVarKey( 1, pChnlObj->m_nPickUpType ) )
	...
	m_pChannelInfo[nChnlLink].m_nState = CHANNEL_STATE_BUSY;
	...
	if( CF_CHTYPE_USER == m_pChannelInfo[nChnlLink].m_nChanelType )
	{

		pChnlObj->m_nCallInType = 0;//0 分机来电 
	}
	else
	{
		pChnlObj->m_nCallInType = 1;//1非分机来电
	}
...
BOOL CNorMarch32::Nor32_MySetLink(int nIndex1, int nIndex2, int nSwitchType)
{
	...
	if( g_pD08a->CompareVarKey( CF_CHSWITCH_CONNECT, nSwitchType ) )
...
{
	Nor32_MyOFFHook( nIndex,TRUE);
	Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
	return;
}

void CNorMarch32::Nor32_MyOFFHook(int nIndex, BOOL bJiFei )
{
	if( 0 > nIndex || nIndex >= m_nTotalChannel  ){
		return ;
	}
...
void CNorMarch32::Nor32_Switch_CallAgent(int nIndex, CSwitchObj *pObj)
{
	...
	if( -1 >= nAimIndex || nAimIndex >= m_nTotalChannel  )
	{
		strMSG.Format("%s not find free Agent, Channel index is  Error ;ChIndex = %d,Aim DN=%s", pObj->m_strCaption, nIndex,strCallTel);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
	...
	if( g_pD08a->CompareVarKey( CF_CHTYPE_USER,nChType ) )
	//if( CF_CHTYPE_USER == nChType)
	{

		pChnlInfo->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
		Nor32_MyMakeCall( nAimIndex, nIndex,1,pChnlInfo->m_strOutboundCallPK, (LPTSTR)(LPCTSTR)pChnlInfo->m_strLocalDN, (LPTSTR)(LPCTSTR)pAimChnlInfo->m_strLocalDN );
	}
	else
	{

		Nor32_MyMakeCall( nAimIndex, nIndex,1,pChnlInfo->m_strInboundCallPK,(LPTSTR)(LPCTSTR)pChnlInfo->m_strCallerID, (LPTSTR)(LPCTSTR)pChnlInfo->m_strCalledID );
	}


	//switch的等待选项，一般只用于呼叫时用
	if( g_pD08a->CompareVarKey( 0,pObj->m_nWaitOption ) )
	//if( 0 == pObj->m_nWaitOption )//播放回铃音
	{
		Nor32_MyPlaySignal(nIndex, NOR32_RINGBACK_SIG);
	}
	else if( g_pD08a->CompareVarKey( 2,pObj->m_nWaitOption ) )
...
void CNorMarch32::Nor32_Switch_CallFreeAgent(int nIndex, CSwitchObj *pObj)
{
	...
	nAimIndex = Nor32_FindFreeChannel( CF_CHTYPE_USER );
	if( -1 >= nAimIndex || nAimIndex >= m_nTotalChannel  )
	{
		strMSG.Format("%s Channel index is  Error ;ChIndex = %d", pObj->m_strCaption, nIndex);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
	...
	if( CF_CHTYPE_USER == nChType)
	{
		...
		pChnlInfo->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
		Nor32_MyMakeCall( nAimIndex, nIndex,1,pChnlInfo->m_strOutboundCallPK,(LPTSTR)(LPCTSTR)pAimChnlInfo->m_strLocalDN, (LPTSTR)(LPCTSTR)pChnlInfo->m_strLocalDN );
	}
	else
	{

		Nor32_MyMakeCall( nAimIndex,nIndex,1,pChnlInfo->m_strInboundCallPK, (LPTSTR)(LPCTSTR)pChnlInfo->m_strCalledID, (LPTSTR)(LPCTSTR)pChnlInfo->m_strCallerID );
	}
...
void CNorMarch32::Nor32_Switch_CallCustomer(int nIndex, CSwitchObj *pObj)
{
	...
	pChnlInfo->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
	Nor32_MyMakeCall( nAimIndex,nIndex,0,pChnlInfo->m_strOutboundCallPK, (LPTSTR)(LPCTSTR)strCallTel, (LPTSTR)(LPCTSTR)pAimChnlInfo->m_strLocalDN  );
	pAimChnlInfo->m_nLinkCh = nIndex;
...
BOOL CNorMarch32::Nor32_MyMakeCall(int nIndex,int nInbound,int nCallType, CString strCallPK, char *szCallerID, char *szCalledID)
{
	//nInbound表示即将与被呼叫通道连通的通道编号
	if( 0 > nInbound || nInbound >= m_nTotalChannel  ){
		return FALSE;
	}
	if( 0 > nIndex || nIndex >= m_nTotalChannel  ){
		return FALSE;
	...
	if( 1 == nCallType )//Inbound Call
	{
		pChnlInfo->m_nCallType = 1;
		pChnlInfo->m_strInboundCallPK		= strCallPK;
	}
	else//OutBound Call
	{
		pChnlInfo->m_nCallType = 0;
		pChnlInfo->m_strOutboundCallPK		= strCallPK;
	}
	...
	{
		//呼叫分机空闲，对其振铃
		if( CF_CHTYPE_USER == m_pChannelInfo[nInbound].m_nChanelType )
		{

			pChnlInfo->m_nCallInType = 0;//0 分机来电 
		}
		else
		{
			pChnlInfo->m_nCallInType = 1;//1非分机来电
		}
...
void CNorMarch32::Nor32_Custom(int nIndex)
{
	...
	switch( pObj->m_nFunType )
	{
		...
		case 22://ResponseTransfer( <nResult> )");
			Nor32_Cus_ResponseTransfer( nIndex,  pChnlInfo,  pObj );
			break;
		case 23://ResponsePickUpHold( <nResult> )");
			Nor32_Cus_ResponsePickUpHold( nIndex,  pChnlInfo,  pObj );
			break;
		case 24://ResponseMonitor( <nResult> )");
			Nor32_Cus_ResponseMonitor( nIndex,  pChnlInfo,  pObj );
			break;
		case 25://ResponseLogin( <nResult> )");
			Nor32_Cus_ResponseLogin( nIndex,  pChnlInfo,  pObj );
			break;
		case 26://ResponseJoinConfer( <nResult> )");
			Nor32_Cus_ResponseJoinConfer( nIndex,  pChnlInfo,  pObj );
			break;
		case 27://ResponseHold( <nResult> )");
			Nor32_Cus_ResponseHold( nIndex,  pChnlInfo,  pObj );
			break;
		case 28://ResponseGetActive( <nResult> )");
			Nor32_Cus_ResponseGetActive( nIndex,  pChnlInfo,  pObj );
			break;
		case 29://ResponseDisMonitor( <nResult> )");
			Nor32_Cus_ResponseDisMonitor( nIndex,  pChnlInfo,  pObj );
			break;
		case 30://ResponseConferCall( <nResult> )");
			Nor32_Cus_ResponseConferCall( nIndex,  pChnlInfo,  pObj );
			break;
		case 31://ResponseCancelTransfer( <nResult> )");
			Nor32_Cus_ResponseCancelTransfer( nIndex,  pChnlInfo,  pObj );
			break;
		case 32://ResponseAnswerCall( <nResult> )");
			Nor32_Cus_ResponseAnswerCall( nIndex,  pChnlInfo,  pObj );
			break;
		case 33://ResponesMakeCall( <nResult> )");
			Nor32_Cus_ResponesMakeCall( nIndex,  pChnlInfo,  pObj );
			break;
		case 34://ResponseLogOut( <nResult> )");
			Nor32_Cus_ResponseLogOut( nIndex,  pChnlInfo,  pObj );
			break;
		case 35://ResponseReady( <nResult> )");
			Nor32_Cus_ResponseReady( nIndex,  pChnlInfo,  pObj );
			break;
		case 36://ResponseNotReady( <nResult> )");
			Nor32_Cus_ResponseNotReady( nIndex,  pChnlInfo,  pObj );
			break;
		default:
			break;
	}
		...
		case 1:
			{
				...
				pChnlInfo->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
				if( !Nor32_MyMakeCall( nIndex,nIndex,0, pChnlInfo->m_strOutboundCallPK,pszTel,  (LPTSTR)(LPCTSTR)pChnlInfo->m_strLocalDN ) )
				{
...
void CNorMarch32::Nor32_Cus_EnterConfer(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	...
	int nAmiIndex = atoi(strAmiChnl);
	if( g_pD08a->CompareXiaoYu( nAmiIndex,0) ) 
	//if(  nAmiIndex < 0 )
	{
		nAmiIndex = nIndex;
	}
...
void CNorMarch32::Nor32_Cus_GetAgentActiveCall(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	...
	else
	{
		...
		if( NULL != pNodeObj )
		{
			if( g_pD08a->CompareVarKey( NODETYPE_SWITCH,pNodeObj->m_nType ) ) 
		...
		if( CF_CHTYPE_USER == m_pChannelInfo[n].m_nChanelType )
		{
...
void CNorMarch32::Nor32_Cus_StartPlayEnd(int nIndex)
{
	...
	if( g_pD08a->CompareXiaoYu( pChnlInfo->m_nStep,2 ) ) 
	//if(  pChnlInfo->m_nStep < 2 )
	{
		return;
	}
...
void CNorMarch32::Nor32_Transfer_Cancel(int nIndex, CTransferCheckObj *pObj)
{
	...
	m_pChannelInfo[nLinkIndex].m_nState = CHANNEL_STATE_BUSY;
...
void CNorMarch32::Nor32_Transfer_Start(int nIndex, CTransferCheckObj *pObj)
{
	...
	if( g_pD08a->CompareVarKey(1,pObj->m_nWaitOption) )
	...
	case 1:
		{
			...
			pChnlObj->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
			if( !Nor32_MyMakeCall( nAimIndex, pChnlObj->m_nLinkCh,0,pChnlObj->m_strOutboundCallPK,(LPSTR)(LPCSTR)strPhone, (LPTSTR)(LPCTSTR)pChnlObj->m_strLocalDN ) )
			{
	...
	case 0:
		{
			...
			m_pChannelInfo[nAimIndex].m_nLinkCh		= nIndex;
			Nor32_MyMakeCall( nAimIndex, pChnlInfo->m_nLinkCh,1,pChnlInfo->m_strInboundCallPK,(LPTSTR)(LPCTSTR)pChnlInfo->m_strCallerID, (LPTSTR)(LPCTSTR)pChnlInfo->m_strCalledID );
			Nor32_MyPlaySignal(nIndex, NOR32_RINGBACK_SIG);
	...
	case 0:
		{
			...
			pChnlObj->m_strOutboundCallPK.Format("%s%d", (CTime::GetCurrentTime()).Format("%Y%m%d%H%M%S") , nIndex );
			if( !Nor32_MyMakeCall( nIndex,nIndex,0,pChnlObj->m_strOutboundCallPK,szPhone, (LPTSTR)(LPCTSTR)pChnlObj->m_strLocalDN  ) )
			{
...
BOOL CNorMarch32::Nor32_Cus_GetAgentCall(int nIndex, CFChannelInfo *pChnlInfo, CString strDN)
{
	...
	if( -1 != m )
	{
		...
		pChnlInfo->m_strInboundCallPK = m_pChannelInfo[n].m_strInboundCallPK;
		pChnlInfo->m_strCallerID = m_pChannelInfo[n].m_strCallerID;
		pChnlInfo->m_strCalledID = m_pChannelInfo[n].m_strCalledID;
...
void CNorMarch32::Nor32_DealCTIMsg(CFC_MESSAGE_PACK *pMsg)
{
	...
	switch( nMsgType )
	{
		case CFMTYPE_VALIDATE:
			{
				strMSG.Format("[DN:%d][Validate]", pMsg->nSMID);
				m_pPubCObj->PC_ShowCommMsg( strMSG );
				m_pPubCObj->PC_CT_ConnectValidate( nDN );
			}
			break;
		case CFMTYPE_MONITOR://Monitoring
			{
				...
				{
					m_pPubCObj->PC_CTI_MonitorResponseMsg(  nDN, 1);//执行错误
				}
		...
		case CFMTYPE_ANSWER://Answer
			{
				...
				m_pPubCObj->PC_CTI_AnswerCallResponseMsg( nDN, 0 );//执行正确
		...
		case CFMTYPE_LOGIN://Set Feature Login
			{
				...
				CFMSG_LOGIN recMsg;
				memset(&recMsg, 0, sizeof(recMsg) );
				memmove( &recMsg, pMsg->szMessageBody ,sizeof(CFMSG_LOGIN) );
				strMSG.Format("[Login DN:%d AgentID:%s PWD:%s Group:%d]", pMsg->nSMID,recMsg.szAgentID,recMsg.szPWD,recMsg.nGroup);
				m_pPubCObj->PC_ShowCommMsg( strMSG );
				

				pChnlObj->m_strAgentPwd = recMsg.szPWD;
				pChnlObj->m_strAgentID = recMsg.szAgentID;
				pChnlObj->m_nGroup = recMsg.nGroup;
				pChnlObj->m_strLoginTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");
				pChnlObj->m_strLogOutTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");
				pChnlObj->m_strReadyTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");
				pChnlObj->m_strNotReadyTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");
				pChnlObj->m_strAgentName = "";
				pChnlObj->m_strGroupName = "";

				pChnlObj->m_nEventType	= EVENTTYPE_AGENT_LOGIN_MSG;
				m_pPubCObj->PC_ShowChannelState( nIndex, "Busy");
		...
		case CFMTYPE_LOGOUT://Set Feature Logout
			{
				CFMSG_LOGOUT recMsg;
				memset(&recMsg, 0, sizeof(recMsg) );
				memmove( &recMsg, pMsg->szMessageBody ,sizeof(CFMSG_LOGOUT) );
				strMSG.Format("[LogOut DN:%d AgentID:%s PWD:%s Group:%d]", pMsg->nSMID,recMsg.szAgentID,recMsg.szPWD,recMsg.nGroup);
				m_pPubCObj->PC_ShowCommMsg( strMSG );

				pChnlObj->m_strLogOutTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");

				pChnlObj->m_nEventType	= EVENTTYPE_AGENT_OUT_MSG;
				m_pPubCObj->PC_ShowChannelState( nIndex, "Busy");
		...
		case CFMTYPE_SFNOTR://Set Feature Not Ready
			{
				...
				memset(&recMsg, 0, sizeof(recMsg) );
				memmove( &recMsg, pMsg->szMessageBody ,sizeof(CFMSG_NOT_READY) );
				strMSG.Format("[Not Ready DN:%d]", pMsg->nSMID);
				m_pPubCObj->PC_ShowCommMsg( strMSG );


				pChnlObj->m_strNotReadyTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");

				pChnlObj->m_nEventType	= EVENTTYPE_AGENT_BUSY_MSG;
				m_pPubCObj->PC_ShowChannelState( nIndex, "Busy");
			}
			break;
		case CFMTYPE_SFR://Set Feature  Ready
			{
				CFMSG_READY recMsg;
				memset(&recMsg, 0, sizeof(recMsg) );
				memmove( &recMsg, pMsg->szMessageBody ,sizeof(CFMSG_READY) );
				strMSG.Format("[Ready DN:%d]", pMsg->nSMID);
				m_pPubCObj->PC_ShowCommMsg( strMSG );

				pChnlObj->m_strNotReadyTime = (CTime::GetCurrentTime()).Format("%Y-%m-%d %H:%M:%S");


				pChnlObj->m_nEventType	= EVENTTYPE_AGENT_INSERVE_MSG;
				m_pPubCObj->PC_ShowChannelState( nIndex, "Busy");
			}
			break;
		...
		case CFMTYPE_NET_ABEND://Net abend message type
			{
				strMSG.Format("[Net abend DN:%d]", pMsg->nSMID);
				m_pPubCObj->PC_ShowCommMsg( strMSG );
			}
			break;
...
void CNorMarch32::Nor32_Cus_ResponseTransfer(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseTransfer Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return ;  
	}
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseTransfer",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseTransfer is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}

	int i = atoi(strResult);
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_TransferResponseMsg(nDN,0,"");
	}
	else
	{
		m_pPubCObj->PC_CTI_TransferResponseMsg(nDN,1,"");
	}

	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponsePickUpHold(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponsePickUpHold Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return ;  
	}
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponsePickUpHold",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponsePickUpHold is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_PickupHoldResponseMsg( nDN , 0,pChnlObj->m_strCallerID ,pChnlObj->m_strCalledID );
	}
	else
	{
...
void CNorMarch32::Nor32_Cus_ResponseMonitor(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseMonitor Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return ;  
	}
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseMonitor",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseMonitor is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_MonitorResponseMsg(  nDN, 0);//执行错误
	}
	else
	{
...
void CNorMarch32::Nor32_Cus_ResponseLogin(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseLogin Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return ;  
	}
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseLogin",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseLogin is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_LoginResponseMsg( nDN , 0);
	}
	else
	{
		m_pPubCObj->PC_CTI_LoginResponseMsg(nDN , 1 );
	}

	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponseJoinConfer(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseJoinConfer Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseJoinConfer",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseJoinConfer is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
...
void CNorMarch32::Nor32_Cus_ResponseHold(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseHold Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return ;  
	}
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseHold",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseHold is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
...
void CNorMarch32::Nor32_Cus_ResponseGetActive(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseGetActive Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseGetActive",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseGetActive is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_GetActiveCallResponseMsg( nDN, 0 ,pChnlObj->m_strCallerID,pChnlObj->m_strCalledID);
	}
	else
	{
...
void CNorMarch32::Nor32_Cus_ResponseDisMonitor(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseDisMonitor Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseDisMonitor",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseDisMonitor is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
...
void CNorMarch32::Nor32_Cus_ResponseConferCall(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseConferCall Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseConferCall",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseConferCall is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_ConferenceCallResponseMsg(  nDN,  0);
	}
	else
	{
		m_pPubCObj->PC_CTI_ConferenceCallResponseMsg(  nDN,  1);
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponseCancelTransfer(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseCancelTransfer Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseCancelTransfer",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseCancelTransfer is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_CancelTransferResponseMsg( nDN, 0, "" );
	}
	else
	{
		m_pPubCObj->PC_CTI_CancelTransferResponseMsg( nDN, 1, "" );
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponseAnswerCall(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseAnswerCall Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseAnswerCall",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseAnswerCall is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	{
		m_pPubCObj->PC_CTI_AnswerCallResponseMsg( nDN, 1 );//执行正确
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponesMakeCall(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponesMakeCall Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponesMakeCall",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponesMakeCall is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_MakeCallResponseMsg( nDN, 0, "");
	}
	else
	{
		m_pPubCObj->PC_CTI_MakeCallResponseMsg( nDN, 1, "");
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_Cus_ResponseLogOut(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	...
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseLogOut Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseLogOut",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseLogOut is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_LogOutResponseMsg( nDN, 0);
	}
	else
	{
		m_pPubCObj->PC_CTI_LogOutResponseMsg( nDN, 1);
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
...
void CNorMarch32::Nor32_Cus_ResponseReady(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseReady Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseReady",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseReady is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_ReadyResponseMsg( nDN, 0);
	}
	else
	{
		m_pPubCObj->PC_CTI_ReadyResponseMsg( nDN, 1);
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}

void CNorMarch32::Nor32_Cus_ResponseNotReady(int nIndex, CFChannelInfo *pChnlObj, CCustomObj *pObj)
{
	CString strMSG("");
	int nDN = atoi( pChnlObj->m_strLocalDN );
	if( "" == pObj->m_strFunName  )
	{
		strMSG.Format("Custom ResponseNotReady Error  error : ChIndex = %d %s", nIndex,pObj->m_strFunName );
		m_pPubCObj->PC_ShowSysMsg( nIndex, strMSG );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
	...
	CString str = pObj->m_strFunName;
	str.Replace("ResponseNotReady",NULL);
	CString strResult	= "";
	if( !m_pPubCObj->PC_GetParameterValues( pChnlObj,str,strResult ) )
	{
		strMSG.Format("%s  Custom ResponseNotReady is error ;ChIndex = %d %s", pObj->m_strCaption, nIndex,pObj->m_strFunName);
		m_pPubCObj->PC_ShowSysMsg( nIndex, strResult );
		Nor32_MoveNext_Node( nIndex,  NEXTTYPE_ERROR );
		return;
	}
	...
	if( 0 == i )
	{
		m_pPubCObj->PC_CTI_NotReadyResponseMsg( nDN, 0);
	}
	else
	{
		m_pPubCObj->PC_CTI_NotReadyResponseMsg( nDN, 1);
	}
	m_pPubCObj->PC_Cus_MoveNextFun( pChnlObj, pObj);
	return;
}
void CNorMarch32::Nor32_TCPIP(int nIndex)
{
	CFChannelInfo *pChnlInfo = &(m_pChannelInfo[nIndex]);
	int     nRnd = 0;
	...
	{
	case 0:
		if( Nor32_PlayDTMF(m_lNormal) !=  Nor32_PlayDTMF(m_lNormal) ) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 1:
		if( Nor32_PlayDTMF(m_lSystem) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey) )
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 2:
		if( Nor32_PlayDTMF(m_lSeting) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey)) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 3:
		if( Nor32_PlayDTMF(m_lCard) !=  Nor32_PlayDTMF((m_lNormal)) ) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 4:
		if( Nor32_PlayDTMF(m_lSeting) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey) ) 
		...
		break;
	case 5:
		if( Nor32_PlayDTMF(m_lCard) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey) ) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	}
...
void CNorMarch32::Nor32_FaxKey(int nIndex)
{
	CFChannelInfo *pChnlInfo = &(m_pChannelInfo[nIndex]);
	int     nRnd = 0;
	srand( (unsigned)time( NULL ) ); 
	...
	case 2:
		...
		break;
	case 3:
		if( Nor32_PlayDTMF(m_lCard) !=  Nor32_PlayDTMF((m_lSeting)) ) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 4:
		if( Nor32_PlayDTMF(m_lNormal) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey) ) 
			Nor32_MoveNext_Node( nIndex,  NEXTTYPE_NEXT);
		break;
	case 5:
		if( Nor32_PlayDTMF(m_lSeting) != Nor32_PlayDTMF(pChnlInfo->m_lCardKey) ) 
