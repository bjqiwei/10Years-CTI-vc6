// CFControlBar.cpp: implementation of the CCFControlBar class.
//
//////////////////////////////////////////////////////////////////////
...
CCFControlBar::CCFControlBar()
{
    ...
    m_rectBorder.SetRect(0,0,0,0);
    m_rectTracker.SetRect(0,0,0,0);
    m_ptOld = CPoint(0,0);
...
IMPLEMENT_DYNAMIC(CCFControlBar, CControlBar)

BEGIN_MESSAGE_MAP(CCFControlBar, CControlBar)
	//{{AFX_MSG_MAP(CCFControlBar)
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_WM_SETCURSOR()
...
/////////////////////////////////////////////////////////////////////////////
// CCFControlBar message handlers

void CCFControlBar::OnUpdateCmdUI(CFrameWnd * pTarget, BOOL bDisableIfNoHndler)
{
    UpdateDialogControls(pTarget, bDisableIfNoHndler);
}

BOOL CCFControlBar::Create(CWnd * pParentWnd, UINT nID, LPCTSTR lpszWindowName, CSize sizeDefault, DWORD dwStyle)
{
...
CSize CCFControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    CRect rc;
...
CSize CCFControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CCFControlBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
	...
	if (m_bButtons)
	{
		...
		if (IsHorzDocked()) {
			...
			m_btnMinim.SetIcon(m_pImageList->ExtractIcon(2),CSize(13,13));
		}
		else {
			rcClose.OffsetRect(14,0);
			m_btnMinim.SetIcon(m_pImageList->ExtractIcon(1),CSize(13,13));
		}
...
BOOL CCFControlBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
BOOL CCFControlBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

BOOL CCFControlBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

void CCFControlBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
void CCFControlBar::OnPaint() 
{
	CPaintDC dc(this); // device context for painting
	DrawGripper(&dc);
}

void CCFControlBar::OnNcPaint() 
{
...
UINT CCFControlBar::OnNcHitTest(CPoint point) 
{
    if (IsFloating())
...
BOOL CCFControlBar::IsFloating()
{
	return (!IsHorzDocked() && !IsVertDocked());
}

void CCFControlBar::StartTracking()
{
    SetCapture();
...
void CCFControlBar::StopTracking(BOOL bAccept)
{
    OnInvertTracker(m_rectTracker);
...
void CCFControlBar::OnInvertTracker(const CRect &rect)
{
    ASSERT_VALID(this);
...
void CCFControlBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void CCFControlBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    if (m_bTracking)
		return;
...
void CCFControlBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CCFControlBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // start the drag
...
void CCFControlBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (!m_bTracking)
...
void CCFControlBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
CPoint& CCFControlBar::ClientToWnd(CPoint &point)
{
    point.Offset(2, 2);
...
CRect CCFControlBar::GetButtonRect()
{
	CRect pRect;
	GetClientRect(pRect);
...
CRect CCFControlBar::GetGripperRect()
{
	CRect pRect;
	GetClientRect(pRect);
	pRect.OffsetRect(-pRect.left,-pRect.top);
...
void CCFControlBar::DrawGripper(CDC* pDC)
{
	if (IsFloating())
		return;
...
int CCFControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (m_bButtons)
	{
		...
		{
			TRACE0("Unable to create CCFControlBar close button\n");
			return -1;
		}
		...
		m_btnClose.SetIcon(m_pImageList->ExtractIcon(0),CSize(13,13));
...
void CCFControlBar::SetBtnImageList(CImageList *pImageList)
{
...
void CCFControlBar::OnButtonClose()
{
	GetDockingFrame()->ShowControlBar(this, FALSE, FALSE);
}

void CCFControlBar::OnUpdateButtonClose(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(true);
}

void CCFControlBar::OnButtonMinimize()
{
	m_pDockContext->ToggleDocking();
}

void CCFControlBar::OnUpdateButtonMinimize(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(true);
}

BOOL CCFControlBar::PreTranslateMessage(MSG* pMsg) 
{
	if (m_bButtons)
...
void CCFControlBar::SetMenuID(UINT nID)
{
	m_menuID = nID;
}

UINT CCFControlBar::GetMenuID()
{
	return m_menuID;
}

void CCFControlBar::OnContextMenu(CWnd* /*pWnd*/, CPoint point) 
{
