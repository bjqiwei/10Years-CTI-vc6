...
public:
	...
	CVipCallSetObjList *m_pVipCallSetObjList;
private:
...
public:
	BOOL PC_Tiff2Bfx( CString &filename );
	BOOL PC_Bfx2Tiff( CString &filename );
	BOOL PC_GetSplitString(CString strOld, CString &str1, CString &str2 );
	void PC_CTI_InserverResponseMsg(int nDN , int nResult );
	void PC_CTI_SetBusyResponseMsg(int nDN , int nResult );
	void PC_ChangeLineState( CFChannelInfo *pChnlInfo, int nState );
	BOOL PC_InitVipConfig( void );
	CVipCallSetObj * PC_GetVipCallSetObj( CString strTel );
	void PC_ShowChannelConf(int nIndex, int nDsp, int nID);
	void PC_ShowChannelFax(int nIndex, int nDsp, int nID );
	void PC_ShowChannelVoice(int nIndex, int nDsp, int nID );
	void PC_ShowXmsEvent( CString strMSG );
