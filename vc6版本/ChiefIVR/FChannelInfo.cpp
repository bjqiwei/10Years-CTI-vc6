...
void CFChannelInfo::MoveFirst_Node()
{
	...
	m_nHoldChnl		= -1;
	m_nInvitConfChnl= -1;
	g_pPubObj->PC_ChangeLineState( this, CHANNEL_STATE_WAITFREE );
	m_nStep			= 0;
	m_lTimeElapse	= 0;
	m_nRings		= 0;
	m_nRecordLength = 0;
...
{
	g_pPubObj->PC_ChangeLineState( this, CHANNEL_STATE_FREE );//在进入CallEndBlock节点前需要将通道状态设置为自由状态
...
BOOL CFChannelInfo::GetContantValues(CString &strValues, int nVarKey)
{
	...
	{
		strValues = m_strSetBusyTime;
		return TRUE;
	}
	else if( 45 == nVarKey )
	{
		strValues = m_strInServerTime;
		return TRUE;
	}
	else
		return FALSE;
